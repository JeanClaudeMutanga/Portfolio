  ...
  <main id="main">
    ...
    <section id="services" class="section-bg ">
      <div class="container">
        ...
        <h4 class="text-warning text-center">No Products Found</h4>
        @else
        <table class="table table-striped mt-5 text-muted ">
            <thead>
                <tr>
                <th scope="col">Name </th>
                <th scope="col">Date Added</th>
                <th scope="col">Owner</th>
                <th scope="col">Categories Options</th>
                
                </tr>
            </thead>
            @foreach(auth()->user()->category as $categories)
            <tbody class= "text-muted">
                <tr>
                <td scope="row"><strong>{{$categories->name}}</strong></td>
                <td>{{$categories->created_at->diffForHumans()}}</td>
                <td>{{$categories->user->name}}</td>
                <td>
                <div class="d-flex">
                  
                  <a href="/categoryedit/{{$categories->id}}" class="btn btn-warning btn-sm mr-2">Edit</a>

                  <form action="/categorydelete/{{$categories->id}}" method = "POST">
                    @csrf
                    @method('DELETE')
                    <button role="button" type="submit" class="btn btn-danger btn-sm ">Delete</button>
                  </form>
